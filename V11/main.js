...
$(document).ready(function() {
    ...
    if ($('#static').attr('checked')){
        // checkbox function goes here (brainstorming)
    }
            ...
            if (($(".selected").length + $(".wrong").length) >= totalSelected) {
                ...
                if ($(".wrong").length == 0) {
                    if((cols+rows)%4 == 0 || (cols+rows)%4 == 1 || (cols+rows)%4 == 2){
                        $(".content").html("<div style = 'position: absolute; left:750px; top: 265px'> <img class = 'animated lightSpeedIn' src = 'nyancat.gif' height = '80' width = '140'> </div>");
